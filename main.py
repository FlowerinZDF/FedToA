...
def _maybe_init_wandb(args, curr_time):
    """Initialize wandb only when explicitly enabled.

    Returns:
        wandb module object if successfully initialized, else None.
    """
    if not getattr(args, "use_wandb", False):
        print("[INFO] wandb disabled; proceeding without wandb.")
        return None

    if wandb is None:
        print("[WARN] --use_wandb was passed, but wandb is not installed. Proceeding without wandb.")
        return None
    ...
    if not run_name:
        run_name = (
            f"{args.exp_name}"
            f"{'_aux' if args.with_aux else ''}"
            f"{'_attn' if args.with_aux and args.aux_attn_only else ''}"
            f"{'_mlp' if args.with_aux and args.aux_mlp_only else ''}"
            f"{'_' + str(args.aux_trained) if args.with_aux else ''}"
            f"_{args.shared_param}"
            f"_{args.share_scope}"
            f"{'_comp' if args.compensation else ''}"
            f"_{args.colearn_param}"
            f"_{args.warmup_modality}"
            f"_{args.freeze_modality}"
            f"_{curr_time}"
        )

    try:
        # Build kwargs safely. Do not pass entity if empty.
        init_kwargs = {
            "config": vars(args),
        ...
        print(f"[INFO] wandb initialized: project={args.wandb_project}, run={run_name}")
        return wandb
    except Exception as e:
        print(f"[WARN] wandb init failed: {e}")
        print("[WARN] Proceeding without wandb.")
        return None
...
def main(args, writer):
    ...
    Args:
        ...
        writer: wandb module object or None
    """
...
if __name__ == "__main__":
    ...
    # wandb arguments      #
    ########################
    parser.add_argument('--use_wandb', help='enable wandb logging', action='store_true')
    parser.add_argument('--wandb_project', type=str, default='fedtoa', help='wandb project name (used only if --use_wandb)')
    parser.add_argument('--wandb_entity', type=str, default=None, help='wandb entity/team name (used only if --use_wandb)')
    parser.add_argument('--wandb_run_name', type=str, default=None, help='optional explicit wandb run name')
    ...
    parser.add_argument('--modalities', help='''name of modalities to use for an experiment (NOTE: case sensitive)
     for multi-task learning.
    ''', nargs='+', type=str)
    parser.add_argument('--Ks', help='number of total cilents participating in federated training for each task)', type=int, default=1, nargs='+')
    parser.add_argument('--Cs', help='number of total cilents participating in federated training for each task)', type=float, default=0.25, nargs='+')
    ...
    # FedCola arguments    #
    ########################
    parser.add_argument('--warmup_modality', type=str, default='none', help='warm up modality')
    parser.add_argument('--warmup_rounds', type=int, default=5, help='warm up rounds')
    parser.add_argument('--freeze_modality', type=str, default='none', help='warm up modality')
    parser.add_argument('--freeze_rounds', type=int, default=5, help='warm up rounds')
    ...
    parser.add_argument('--pub_anno_path', type=str, default='data/coco/annotations/captions_val2014.json', help='public annotation directory')
    parser.add_argument('--num_pub_samples', type=int, default=500, help='public samples number')
    parser.add_argument('--pub_batch_size', type=int, default=64, metavar='N', help='input batch size for training (default: 64)')
    parser.add_argument('--p_lr', type=float, default=1e-4, metavar='LR', help='learning rate (default: 0.001)')
    parser.add_argument('--interintra_weight', type=float, default=0.5, help='inter intra loss weight')
    ...
    # Model arguments #
    ###################
    parser.add_argument('--model_name', help='a model to be used (NOTE: case sensitive)', type=str, required=True)
    ...
    # Learning arguments #
    ######################
    parser.add_argument('--algorithm', help='federated learning algorithm to be used', type=str, required=True)
    ...
    ''', type=str, choices=['local', 'global', 'both'], required=True)
    ...
    if len(args.out_modality_scales) == 1 and args.modalities is not None:
    ...
    # define writer-like object (wandb or None)
